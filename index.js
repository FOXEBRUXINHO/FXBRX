	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hello@${num.split('@')[0]}\nBem vindo ao grupo, olhe as regras pra não ser removido *${mdata.subject}*`
	...
	client.on('chat-update', async (mek) => {
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Funciona Promote @${mentioned[0].split('@')[0]} Como Administrador de Grupo!`, mentioned, true)
